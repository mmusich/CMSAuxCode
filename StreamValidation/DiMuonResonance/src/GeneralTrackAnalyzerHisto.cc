...
class GeneralTrackAnalyzerHisto : public edm::EDAnalyzer {
 public:
  ...
  virtual void analyze(const edm::Event& event, const edm::EventSetup& setup){
    ...
    setup.get<TransientTrackRecord>().get("TransientTrackBuilder",theB_);
	...
	hHitComposition->Fill(5.,track->hitPattern().numberOfValidStripTECHits());
      ...
      hPt->Fill(track->pt());
      hPtZoom->Fill(std::max(track->pt(),9.99));
      hP->Fill(track->p());
	...
	htrkQuality->Fill(myquality);
      ...
      // Fill track profiles
      static const int d0phiindex = this->GetIndex(vTrackProfiles_,"prof_d0_vs_phi");
      vTrackProfiles_[d0phiindex]->Fill(track->phi(),track->d0());
      static const int dzphiindex = this->GetIndex(vTrackProfiles_,"prof_dz_vs_phi");
      vTrackProfiles_[dzphiindex]->Fill(track->phi(),track->dz());
      static const int d0etaindex = this->GetIndex(vTrackProfiles_,"prof_d0_vs_eta");
      vTrackProfiles_[d0etaindex]->Fill(track->eta(),track->d0());
      static const int dzetaindex = this->GetIndex(vTrackProfiles_,"prof_dz_vs_eta");
      vTrackProfiles_[dzetaindex]->Fill(track->eta(),track->dz());
      static const int chiProbphiindex = this->GetIndex(vTrackProfiles_,"prof_chi2Prob_vs_phi");
      vTrackProfiles_[chiProbphiindex]->Fill(track->phi(),chi2Prob);
      static const int chiProbabsd0index = this->GetIndex(vTrackProfiles_,"prof_chi2Prob_vs_d0");
      vTrackProfiles_[chiProbabsd0index]->Fill(fabs(track->d0()),chi2Prob);
      static const int chiProbabsdzindex = this->GetIndex(vTrackProfiles_,"prof_chi2Prob_vs_dz");
      vTrackProfiles_[chiProbabsdzindex]->Fill(track->dz(),chi2Prob);
      static const int chiphiindex = this->GetIndex(vTrackProfiles_,"prof_chi2_vs_phi");
      vTrackProfiles_[chiphiindex]->Fill(track->phi(),track->chi2());
      static const int normchiphiindex = this->GetIndex(vTrackProfiles_,"prof_normchi2_vs_phi");
      vTrackProfiles_[normchiphiindex]->Fill(track->phi(),normchi2);
      static const int chietaindex = this->GetIndex(vTrackProfiles_,"prof_chi2_vs_eta");
      vTrackProfiles_[chietaindex]->Fill(track->eta(),track->chi2());
      static const int normchiptindex = this->GetIndex(vTrackProfiles_,"prof_normchi2_vs_pt");
      vTrackProfiles_[normchiptindex]->Fill(track->pt(),normchi2);
      static const int normchipindex = this->GetIndex(vTrackProfiles_,"prof_normchi2_vs_p");
      vTrackProfiles_[normchipindex]->Fill(track->p(),normchi2);
      static const int chiProbetaindex = this->GetIndex(vTrackProfiles_,"prof_chi2Prob_vs_eta");
      vTrackProfiles_[chiProbetaindex]->Fill(track->eta(),chi2Prob);
      static const int normchietaindex = this->GetIndex(vTrackProfiles_,"prof_normchi2_vs_eta");
      vTrackProfiles_[normchietaindex]->Fill(track->eta(),normchi2);
      static const int kappaphiindex = this->GetIndex(vTrackProfiles_,"prof_kappa_vs_phi");
      vTrackProfiles_[kappaphiindex]->Fill(track->phi(),kappa);
      static const int kappaetaindex = this->GetIndex(vTrackProfiles_,"prof_kappa_vs_eta");
      vTrackProfiles_[kappaetaindex]->Fill(track->eta(),kappa);
      static const int ptResphiindex = this->GetIndex(vTrackProfiles_,"prof_ptResolution_vs_phi");
      vTrackProfiles_[ptResphiindex]->Fill(track->phi(),track->ptError()/track->pt());
      static const int ptResetaindex = this->GetIndex(vTrackProfiles_,"prof_ptResolution_vs_eta");
	...
	// std::cout<<"n. vertices "<<vertexCollection.size()<<std::endl;
	pvtx = vertexHandle->at(0);
      	for(reco::VertexCollection::const_iterator ipvtx = vertexHandle->begin(); ipvtx!=vertexHandle->end(); ++ipvtx) {
      	  math::XYZPoint mypoint( ipvtx->x(), ipvtx->y(),ipvtx->z());
	...
	fillTrackHistos(hDA,"all",&(*track),pvtx,beamSpot,fBfield_,theB_,
			[](const reco::Track*, const reco::Vertex) -> bool { return true; }
			);
	
	fillTrackHistos(hDA,"sel",&(*track),pvtx,beamSpot,fBfield_,theB_,
			[](const reco::Track* track, const reco::Vertex pvtx) -> bool { return track->pt() > 10. && abs(track->dxy(pvtx.position()))/track->dxyError() < 5;}
			);
  ...
  virtual void beginJob() {
    ...
    TFileDirectory DA = fs->mkdir("VertexPlots");
    //DA.cd();
    ...
    hPtZoom = fs->make<TH1D>("hPtZoom","Transverse Momentum;track p_{T} [GeV];tracks",100,0.,10.);
    ...
    hHitCountVsZBPix = fs->make<TH1D>("hHitCountVsZBpix","Number of hits vs z;hit global z;hits",100,-100,100);
    hHitCountVsZFPix = fs->make<TH1D>("hHitCountVsZFpix","Number of hits vs z;hit global z;hits",100,-100,100);
    ...
    pNBpixHitsVsVx = fs->make<TProfile>("prof_NpixHits_vs_Vx",
					"n. Barrel Pixel hits vs. v_{x};v_{x} (cm);n. BPix hits",
					20,-20,20);

    pNBpixHitsVsVy = fs->make<TProfile>("prof_NpixHits_vs_Vy",
					"n. Barrel Pixel hits vs. v_{y};v_{y} (cm);n. BPix hits",
					20,-20,20);

    pNBpixHitsVsVz = fs->make<TProfile>("prof_NpixHits_vs_Vz",
    ...
    vTrackHistos_.push_back(fs->make<TH1F>("h_ptResolution",
					   ...
					   100,0.,0.5));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_d0_vs_phi",
						 "Transverse Impact Parameter vs. #phi;#phi_{Track};#LT d_{0} #GT [cm]",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_dz_vs_phi",
						 "Longitudinal Impact Parameter vs. #phi;#phi_{Track};#LT d_{z} #GT [cm]",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_d0_vs_eta",
						 "Transverse Impact Parameter vs. #eta;#eta_{Track};#LT d_{0} #GT [cm]",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_dz_vs_eta",
						 "Longitudinal Impact Parameter vs. #eta;#eta_{Track};#LT d_{z} #GT [cm]",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_chi2_vs_phi",
						 "#chi^{2} vs. #phi;#phi_{Track};#LT #chi^{2} #GT",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_chi2Prob_vs_phi",
						 "#chi^{2} probablility vs. #phi;#phi_{Track};#LT #chi^{2} probability#GT",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_chi2Prob_vs_d0",
						 "#chi^{2} probablility vs. |d_{0}|;|d_{0}|[cm];#LT #chi^{2} probability#GT",
						 100,0,80));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_chi2Prob_vs_dz",
						 "#chi^{2} probablility vs. dz;d_{z} [cm];#LT #chi^{2} probability#GT",
						 100,-30,30));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_normchi2_vs_phi",
						 "#chi^{2}/ndof vs. #phi;#phi_{Track};#LT #chi^{2}/ndof #GT",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_chi2_vs_eta",
						 "#chi^{2} vs. #eta;#eta_{Track};#LT #chi^{2} #GT",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_normchi2_vs_pt",
						 "norm #chi^{2} vs. p_{T}_{Track}; p_{T}_{Track};#LT #chi^{2}/ndof #GT",
						 18,xBins));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_normchi2_vs_p",
						 "#chi^{2}/ndof vs. p_{Track};p_{Track};#LT #chi^{2}/ndof #GT",
						 18,xBins));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_chi2Prob_vs_eta",
						 "#chi^{2} probability vs. #eta;#eta_{Track};#LT #chi^{2} probability #GT",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_normchi2_vs_eta",
						 "#chi^{2}/ndof vs. #eta;#eta_{Track};#LT #chi^{2}/ndof #GT",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_kappa_vs_phi",
						 "#kappa vs. #phi;#phi_{Track};#kappa",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_kappa_vs_eta",
						 "#kappa vs. #eta;#eta_{Track};#kappa",
						 100,-3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_ptResolution_vs_phi",
						 "#delta_{p_{T}}/p_{T}^{track};#phi^{track};#delta_{p_{T}}/p_{T}^{track}",
						 100, -3.15,3.15));
    vTrackProfiles_.push_back(fs->make<TProfile>("prof_ptResolution_vs_eta",
  ...
  //*************************************************************
  {
    ...
    for(int t=0; t<2; t++){
      ...
      h["trackd0Vert_"+types[t]] = dir.make<TH1F>(("trackd0Vert_"+types[t]).c_str(),"track d_{0} impact parameter;track d_{0} impact parameter;tracks",200,-100.,100.);
      h["trackdzVert_"+types[t]] = dir.make<TH1F>(("trackdzVert_"+types[t]).c_str(),"track d_{0} impact parameter;track d_{z} impact parameter;tracks",200,-100.,100.);  
      h["trackSigd0Vert_"+types[t]] = dir.make<TH1F>(("trackSigd0Vert_"+types[t]).c_str(),"track d_{0} impact parameter significance;track d_{0}/#sigma_{d_{0}};tracks",100,-20.,20.);
      h["trackSigdzVert_"+types[t]] = dir.make<TH1F>(("trackSigdzVert_"+types[t]).c_str(),"track d_{0} impact parameter significance;track d_{z}/#sigma_{d_{z}};tracks",200,-20.,20.);

      h["kappa_"+types[t]]= dir.make<TH1F>(("kappa_"+types[t]).c_str(),"#kappa;#kappa;tracks",200,-20.,20.);
      h["zdcap_"+types[t]]= dir.make<TH1F>(("zdcap_"+types[t]).c_str(),"zdcap;zdcap;tracks",200,-20.,20.);
  ...
  //*************************************************************
  void fillTrackHistos(std::map<std::string, TH1*> & h, const std::string & ttype, 
		       const reco::Track* tt,
		       const reco::Vertex & v, 
		       const reco::BeamSpot & beamSpot, 
		       double fBfield_,
		       edm::ESHandle<TransientTrackBuilder> theB_,
		       std::function<bool(const reco::Track*, const reco::Vertex)> trackFilter
		       )
  //*************************************************************
  {
    ...
    if (!trackFilter(tt,v)) return; 

    fill(h,"pseudorapidity_"+ttype,tt->eta());
    fill(h,"z0_"+ttype,tt->vz());
    fill(h,"phi_"+ttype,tt->phi());
    fill(h,"eta_"+ttype,tt->eta());
    fill(h,"pt_"+ttype,tt->pt());
    fill(h,"p_"+ttype,tt->p());
    fill(h,"found_"+ttype,tt->found());
    fill(h,"lost_"+ttype,tt->lost());
    fill(h,"nchi2_"+ttype,tt->normalizedChi2());
    fill(h,"rstart_"+ttype,(tt->innerPosition()).Rho());
    
    double d0Error=tt->d0Error();
    double d0=tt->dxy(beamSpot.position());
    double dz=tt->dz(beamSpot.position());
    if (d0Error>0){
    ...
    fill(h,"sigmatrkz_"+ttype,sqrt(pow(tt->dzError(),2)+wxy2_/pow(tan(tt->theta()),2)));
    fill(h,"sigmatrkz0_"+ttype,tt->dzError());
    
    // track vs vertex
    if( v.isValid()){ // && (v.ndof()<10.)) {
      // emulate clusterizer input
      //const TransientTrack & tt = theB_->build(&t); wrong !!!!
      reco::TransientTrack ttt = theB_->build(&(*tt)); 
      ttt.setBeamSpot(beamSpot); // need the setBeamSpot !
      double z=(ttt.stateAtBeamLine().trackStateAtPCA()).position().z();
      double tantheta=tan((ttt.stateAtBeamLine().trackStateAtPCA()).momentum().theta());
      double dz2= pow(tt->dzError(),2)+wxy2_/pow(tantheta,2);
      
      fill(h,"restrkz_"+ttype,z-v.position().z());
      fill(h,"restrkzvsphi_"+ttype,tt->phi(), z-v.position().z());
      fill(h,"restrkzvseta_"+ttype,tt->eta(), z-v.position().z());
      fill(h,"pulltrkzvsphi_"+ttype,tt->phi(), (z-v.position().z())/sqrt(dz2));
      fill(h,"pulltrkzvseta_"+ttype,tt->eta(), (z-v.position().z())/sqrt(dz2));
      
      fill(h,"pulltrkz_"+ttype,(z-v.position().z())/sqrt(dz2));
      
      double x1=tt->vx()-beamSpot.x0(); 
      double y1=tt->vy()-beamSpot.y0();
      
      double kappa=-0.002998*fBfield_*tt->qoverp()/cos(tt->theta());
      double D0=x1*sin(tt->phi())-y1*cos(tt->phi())-0.5*kappa*(x1*x1+y1*y1);
      double q=sqrt(1.-2.*kappa*D0);
      double s0=(x1*cos(tt->phi())+y1*sin(tt->phi()))/q;
      
      fill(h,"kappa_"+ttype,kappa);
      
      // with respect to any specified vertex, such as primary vertex
      GlobalPoint vert(v.position().x(),v.position().y(),v.position().z());
      TrajectoryStateClosestToPoint traj = ttt.trajectoryStateClosestToPoint(vert);
      ...
      fill(h,"trackd0Vert_"+ttype,d0);
      fill(h,"trackdzVert_"+ttype,z0);
      fill(h,"trackSigd0Vert_"+ttype,d0/d0_error);
      fill(h,"trackSigdzVert_"+ttype,z0/z0_error);
	...
	s1=s0*(1.+ks02/6.+3./40.*ks02*ks02+5./112.*pow(ks02,3));
      ...
      // sp.ddcap=-2.*D0/(1.+q);
      double zdcap=tt->vz()-s1/tan(tt->theta());
      fill(h,"zdcap_"+ttype,zdcap);
    }
    //
    
    // collect some info on hits and clusters
    fill(h,"nbarrelLayers_"+ttype,tt->hitPattern().pixelBarrelLayersWithMeasurement());
    fill(h,"nPxLayers_"+ttype,tt->hitPattern().pixelLayersWithMeasurement());
    fill(h,"nSiLayers_"+ttype,tt->hitPattern().trackerLayersWithMeasurement());
    fill(h,"expectedInner_"+ttype,tt->hitPattern().numberOfHits(HitPattern::MISSING_INNER_HITS));
    fill(h,"expectedOuter_"+ttype,tt->hitPattern().numberOfHits(HitPattern::MISSING_OUTER_HITS));
    fill(h,"trackAlgo_"+ttype,tt->algo());
    fill(h,"trackQuality_"+ttype,tt->qualityMask());
    
    //
    int longesthit=0, nbarrel=0;
    for(trackingRecHit_iterator hit=tt->recHitsBegin(); hit!=tt->recHitsEnd(); hit++){
	...
	if (barrel){
	  ...
	  if (clust.isNonnull()) {
	    ...
	    if (clust->sizeY()>20.){
	      fill(h,"lvseta_"+ttype,tt->eta(), 19.9);
	      fill(h,"lvstanlambda_"+ttype,tan(tt->lambda()), 19.9);
	    }else{
	      fill(h,"lvseta_"+ttype,tt->eta(), float(clust->sizeY()));
	      fill(h,"lvstanlambda_"+ttype,tan(tt->lambda()), float(clust->sizeY()));
	    }
	  }
	}
      }
