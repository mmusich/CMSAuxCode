...
class TrackAnalyzer : public edm::EDAnalyzer {
 public:
  ...
  virtual void analyze(const edm::Event& event, const edm::EventSetup& setup){
    ...
    run_Event = event.eventAuxiliary().run();
    lumisec_Event= event.eventAuxiliary().luminosityBlock();
    num_Event = event.eventAuxiliary().id().event();
    ...
    branch.run_Event     = run_Event;	 	 
    branch.lumisec_Event = lumisec_Event;  
    branch.num_Event     = num_Event;          
...
//     return; 
  ...
  virtual void beginJob() {
    ...
    AnalysisTree->Branch("runEvent",&branch.run_Event);
    AnalysisTree->Branch("lumisecEvent",&branch.lumisec_Event);
    AnalysisTree->Branch("numEvent",&branch.num_Event);
