...
class ShallowTrackAnalyzer : public edm::EDAnalyzer {
 public:
  ShallowTrackAnalyzer(const edm::ParameterSet& pset) {
  
    TkToken_ = consumes<reco::TrackCollection>(pset.getParameter<edm::InputTag>("TkTag"));
  ...
  virtual void analyze(const edm::Event& event, const edm::EventSetup& setup){
    ...
    event.getByToken(TkToken_,trackCollection);
    ...
    edm::InputTag tag("TriggerResults","","HLT");//InputTag tag("TriggerResults");
  ...
  virtual void beginJob() {
    ...
    ievt=0;
    hrun  = fs->make<TH1D>("h_run","run",1000000,0,1000000);
...
DEFINE_FWK_MODULE(ShallowTrackAnalyzer);
